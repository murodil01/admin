    ...
    Modal.confirm({
      ...
      onOk: async () => {
        try {
          ...
          setDropdownOpenId(null); // dropdown yopish
      ...
      onCancel() {
        // foydalanuvchi bekor qilsa hech narsa qilinmaydi
      },
    });
  };
  ...
  const menuItems = (board) => [
    {
      ...
      onClick: (info) => {
        info.domEvent.stopPropagation(); // dropdown eventni to‘xtatish
        setEditingBoard(board); // modalni ochish
        setDropdownOpenId(null); // dropdown yopish
      },
    },
    {
    ...
    <div className="min-h-[640px] bg-[#F7F7F7] border border-gray-200 rounded-[16px] p-4 max-w-[320px] flex flex-col gap-4">
      ...
      {loading ? (
        ...
        boards.map((board) => (
          <div
            key={board.id}
            className="flex items-center justify-between pr-3 hover:bg-gray-100 rounded-md cursor-pointer"
          ...
          >
            ...
            <Dropdown
              ...
              onOpenChange={(open) => setDropdownOpenId(open ? board.id : null)}
            >
              <div
